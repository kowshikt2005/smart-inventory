...
export default function SalesRegisterDetailsPage() {
  return (
    <Suspense fallback={<DashboardLayout><div className="p-6 flex items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-teal-500" /></div></DashboardLayout>}>
      <SalesRegisterDetailsContent />
    </Suspense>
  );
}

function SalesRegisterDetailsContent() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/reports/sales-register/details?${params}`);
        if (response.ok) {
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-4 gap-4">
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <p className="text-sm text-green-600">Credit (Sales)</p>
                <p className="text-xl font-bold text-green-700">₹{fmt(data.summary.credit)}</p>
              </div>
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                <p className="text-sm text-red-600">Debit (Receipts + Returns)</p>
            ...
            {data.invoices.length > 0 && (
              <div className="bg-white border rounded-lg overflow-hidden">
                <div className="bg-gray-100 px-4 py-3 border-b">
                  <h2 className="font-semibold text-gray-800">Sales Invoices</h2>
                </div>
                    ...
                    <tbody>
                      {data.invoices.map((inv) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-sm font-medium">{inv.customerName}</td>
            ...
            {data.payments.length > 0 && (
              <div className="bg-white border rounded-lg overflow-hidden">
                <div className="bg-blue-100 px-4 py-3 border-b">
                  <h2 className="font-semibold text-blue-800">Receipts</h2>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead>
                    ...
                    <tbody>
                      {data.payments.map((pay) => (
                        <tr
                          key={pay.id}
                          className="border-t hover:bg-blue-50 cursor-pointer"
                          onClick={() => router.push(`/sales/receipts/${pay.id}`)}
                        >
                          ...
                          <td className="px-4 py-3 text-sm">{pay.customerName}</td>
            ...
            {data.returns.length > 0 && (
              <div className="bg-white border rounded-lg overflow-hidden">
                <div className="bg-orange-100 px-4 py-3 border-b">
                  <h2 className="font-semibold text-orange-800">Sales Returns</h2>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead>
