...
export default function PurchaseRegisterPage() {
  ...
  const selectedVendorName =
    vendorId === "all"
      ? "All Vendors"
      : vendors.find((v) => v.id === vendorId)?.name || "All Vendors";
  ...
  const queryString = useMemo(() => {
    ...
    if (vendorId !== "all") p.append("vendorId", vendorId);
    return p.toString();
  ...
  const handleEmailSend = async (emails: string[], fromDate: string, toDate: string) => {
    ...
    if (vendorId !== "all") qs.append("vendorId", vendorId);
    const res = await fetch(`/api/reports/purchase-register?${qs}`);
    ...
    const pdfBase64 = generatePDFBase64({
      title: "Purchase Register",
      subtitle: `${fmtDate(fromDate)} - ${fmtDate(toDate)} | ${selectedVendorName}`,
      sheets: [{ name: "Purchase Register", headers: ["Month", "Gross Amount (₹)", "Tax Amount (₹)", "Net Amount (₹)"], rows }],
      company,
    });
    ...
    const send = await fetch("/api/reports/send-email", {
      ...
      body: JSON.stringify({
        emails,
        subject: `Purchase Register Report — ${fmtDate(fromDate)} to ${fmtDate(toDate)}`,
        pdfBase64,
        filename: `Purchase-Register_${fmtDate(fromDate)}_to_${fmtDate(toDate)}.pdf`,
      ...
      <div className="p-6">
        ...
        <div className="flex justify-center mb-4">
          <div className="relative">
            <button
              onClick={() => setShowVendorDrop(!showVendorDrop)}
              ...
              <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1 bg-white border rounded-lg shadow-lg z-10 w-64 max-h-56 overflow-y-auto">
                <button
                  ...
                  className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-50 ${
                    vendorId === "all" ? "bg-teal-50 font-semibold text-teal-700" : "text-gray-700"
                  }`}
                >
                  All Vendors
                </button>
                {vendors.map((v) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-50 ${
                      vendorId === v.id ? "bg-teal-50 font-semibold text-teal-700" : "text-gray-700"
                    }`}
                  >
                    {v.name}
                  </button>
        ...
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
          {isLoading ? (
            <div className="flex items-center justify-center gap-2 py-16 text-gray-500">
              ...
              <span>Loading purchase register…</span>
            </div>
          ) : (
